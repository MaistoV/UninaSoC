...
#define UART_BASE_ADDR      0x00200000
...
uint8_t uart_is_tx_empty(struct uart_t *);

//Check if the receiver is empty
uint8_t uart_is_rx_empty(struct uart_t *);

// Send a single character to the UART
void uart_send_char(struct uart_t *, uint8_t ch);

// Waits until a character is received on UART (RX) and returns it.
uint32_t uart_get_char(struct uart_t *);
