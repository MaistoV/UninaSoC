...
void initialize_data(
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        for (int j = 0; j < DATA_SIZE; j++) {
            A[i*DATA_SIZE + j] = i*DATA_SIZE + j +1;
            B[i*DATA_SIZE + j] = j*DATA_SIZE + i +1;
...
int main() {
    ...
    XKrnl_MATMUL InstancePtr;
    InstancePtr.Control_BaseAddress = Xkrnl_BASE;
    // if ( XKrnl_MATMUL_Initialize(&InstancePtr, Xkrnl_BASE) != 0 ) {
    //     return 1;
    // }
    ...
    // } while ( XKrnl_MATMUL_IsIdle(&InstancePtr) );
    ...
    XKrnl_MATMUL_EnableAutoRestart(&InstancePtr);
    // Raising ap_start to start the kernel
    // Xil_Out32(Xkrnl_Control, AP_START | AP_AUTORESTART_BIT);
    XKrnl_MATMUL_Start(&InstancePtr);
    ...
    // for ( uint32_t i = 0; i < 1000; i++ ) {
        ...
        // csr_read = Xil_In32(Xkrnl_Control) & AP_AUTORESTART;
        // Xil_Out32(Xkrnl_Control, csr_read | AP_CONTINUE);

        // Dump
        printf("hls_out : 0x%04x:\n\r", (u32)hls_out ); print_matrix(hls_out );
        printf("expected: 0x%04x:\n\r", (u32)expected); print_matrix(expected);
        dump_csrs();
