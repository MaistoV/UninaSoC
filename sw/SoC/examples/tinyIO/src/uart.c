...
struct uart_t * uart_init(uint32_t baseaddr) {
...
__attribute__((always_inline)) inline uint8_t uart_is_rx_empty(struct uart_t * global_uart) {
	return (global_uart->state_reg & UART_RX_FIFO_NOT_EMPTY) ? 0 : 1;
}

// Check if the transmitter is empty
__attribute__((always_inline)) inline uint8_t uart_is_tx_empty(struct uart_t * global_uart) {
	return (global_uart->state_reg & UART_TX_EMPTY) ? 1 : 0;
}


// Send a single character to the UART
void uart_send_char(struct uart_t * global_uart, uint8_t ch) {
...
uint32_t uart_get_char(struct uart_t * global_uart){
